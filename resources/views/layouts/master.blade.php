...
<body class="bg-white text-gray-900 font-sans antialiased dark:bg-gray-900 dark:text-gray-100 {{ Auth::check() ? 'logged-in' : '' }}">
    ...
    <div class="fixed bottom-6 right-6 flex flex-col gap-3 z-50">
        ...
        <a href="tel:{{ $setting->phone ?? '' }}" class="w-12 h-12 flex items-center justify-center bg-green-500 hover:bg-green-600 text-white rounded-full shadow-lg transition-transform hover:scale-110">
           <i class="fas fa-phone-alt animate-pulse"></i>
        </a>
        {{-- Nút Zalo --}}
        <a href="{{ $setting->zalo ?? '' }}" target="_blank" class="w-12 h-12 flex items-center justify-center bg-blue-500 hover:bg-blue-600 text-white rounded-full shadow-lg transition-transform hover:scale-110">
            <i class="fas fa-comment-dots"></i>
        </a>
        {{-- Nút Lên đầu trang (Back to top) --}}
        <a href="#" class="w-12 h-12 hidden items-center justify-center bg-gray-800 hover:bg-gray-700 text-white rounded-full shadow-lg transition-transform hover:scale-110" id="js-back-to-top">
